...
int DiMuonMassSpectrum_muonQA_Ypsilon()
{
    ...
    */
    ...
    // nBins, 0, nBins);
    TH1D *hTemplateWidths = new TH1D("hTemplateWidths", "Y(1S) width vs. p_{T} range; p_{T} (GeV/c); GeV/c^{2}",
                                     nBins, binEdges.data());
    TH1D *hTemplatePeaks = new TH1D("hTemplatePeaks", "Y(1S) peak vs. p_{T} range; p_{T} (GeV/c); GeV/c^{2}",
                                    nBins, binEdges.data());
    hTemplateWidths->SetTitle(Form("%s : Y(1S) width vs. p_{T} range; p_{T} (GeV/c)", labelName));
    hTemplatePeaks->SetTitle(Form("%s : Y(1S) peak vs. p_{T} range; p_{T} (GeV/c)", labelName));
    ...
    for (int i = 0; i < vTreeNames.size(); i++)
    {
        ...
        // nBins, 0, nBins);
        hWidths = new TH1D(Form("hWidths_%s", fileName), "Y width vs. p_{T} range; p_{T} (GeV/c); GeV/c^{2}",
                           nBins, binEdges.data());
        hPeaks = new TH1D(Form("hPeaks_%s", fileName), "Y peak vs. p_{T} range; p_{T} (GeV/c); GeV/c^{2}",
                          nBins, binEdges.data());
        ...
        TCanvas *canvasJpsiWidths = new TCanvas(Form("cYWidths_%s", fileName), Form("cYWidths_%s", fileName), 800, 600);
        canvasJpsiWidths->cd();
        hWidths->SetStats(0);
        hWidths->Draw("PE");
        ...
        TCanvas *canvasJpsiPeaks = new TCanvas(Form("cYPeaks_%s", fileName), Form("cYPeaks_%s", fileName), 800, 600);
        canvasJpsiPeaks->cd();
        hPeaks->SetStats(0);
        hPeaks->Draw("PE");
