...
void fitJpsiCB(TH1D* hist, Double_t pTMin, Double_t pTMax) {
    /*
    TCanvas* test = new TCanvas(Form("test_Pt_%.0f_%.0f", pTMin, pTMax), 
                                Form("test_Pt_%.0f_%.0f", pTMin, pTMax));
    test->cd();
    hist->Draw();
    */
    ...
    RooChebychev* BKG = new RooChebychev(Form("BKG_Pt_%.0f_%.0f", pTMin, pTMax),"Chebyshev background", m, {a0,a1,a2,a3,a4,a5,a6,a7});
...
int DiMuonMassSpectrum() 
 {
    ...
    fitJpsiCB(hDiMuonMass_PtCut_0_2,  0, 2);

    // fitJpsiGauss(hDiMuonMass_PM_Pt_2_5, 2, 5);
    fitJpsiCB(hDiMuonMass_PtCut_3_4,  3, 4);

    // fitJpsiGauss(hDiMuonMass_PM_Pt_5_30, 5, 30);
    fitJpsiCB(hDiMuonMass_PtCut_5_30, 5, 30);
